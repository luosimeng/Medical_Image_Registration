...
# parser.add_argument('--model', default='/data1/lsm/voxelmorph-oasis/models/1500.h5', help='VxmDense model file')
parser.add_argument('--pairs', required=True, help='path to list of image pairs to register')
# parser.add_argument('--pairs', default='Pair_list.txt', help='path to list of image pairs to register')
...
with tf.device(device):
    ...
    for i in range(len(img_pairs)):
        ...
        moving_vol = vxm.py.utils.load_volfile(
            '/data1/ykq/nnUNet_dataset/nnUNet_raw/nnUNet_raw_data/Task011_oasis_data/OASIS_OAS1_'+img_pairs[i][0]+'_MR1/aligned_norm.nii.gz', np_var='vol', add_batch_axis=True, add_feat_axis=add_feat_axis)
        moving_seg = vxm.py.utils.load_volfile(
            '/data1/ykq/nnUNet_dataset/nnUNet_raw/nnUNet_raw_data/Task011_oasis_data/OASIS_OAS1_'+seg_pairs[i][0]+'_MR1/aligned_seg35.nii.gz', np_var='vol', add_batch_axis=True, add_feat_axis=add_feat_axis)

        # load fixed image and seg
        fixed_vol = vxm.py.utils.load_volfile(
            '/data1/ykq/nnUNet_dataset/nnUNet_raw/nnUNet_raw_data/Task011_oasis_data/OASIS_OAS1_'+img_pairs[i][1]+'_MR1/aligned_norm.nii.gz', np_var='vol', add_batch_axis=True, add_feat_axis=add_feat_axis)
        fixed_seg = vxm.py.utils.load_volfile(
            '/data1/ykq/nnUNet_dataset/nnUNet_raw/nnUNet_raw_data/Task011_oasis_data/OASIS_OAS1_'+seg_pairs[i][1]+'_MR1/aligned_seg35.nii.gz', np_var='vol')

        print('PAIR: ',img_pairs[i][0],'-',img_pairs[i][1])
        ...
        # nib.save(new_img,'/data1/lsm/voxelmorph-oasis/try_test_image/img_fixed.nii.gz')
        # new_img=nib.Nifti1Image(moving_vol.squeeze(),np.eye(4))
        # nib.save(new_img,'/data1/lsm/voxelmorph-oasis/try_test_image/img_moving.nii.gz')
        # new_img=nib.Nifti1Image(moving_seg.squeeze(),np.eye(4))
        # nib.save(new_img,'/data1/lsm/voxelmorph-oasis/try_test_image/seg_moving.nii.gz')
        # new_img=nib.Nifti1Image(fixed_seg.squeeze(),np.eye(4))
        # nib.save(new_img,'/data1/lsm/voxelmorph-oasis/try_test_image/seg_fixed.nii.gz')
        # new_img=nib.Nifti1Image(warped_img.squeeze(),np.eye(4))
        # nib.save(new_img,'/data1/lsm/voxelmorph-oasis/try_test_image/warped.nii.gz')
        # new_img=nib.Nifti1Image(warp.squeeze(),np.eye(4))
        # nib.save(new_img,'/data1/lsm/voxelmorph-oasis/try_test_image/flow.nii.gz')
        # new_img=nib.Nifti1Image(warped_seg.squeeze(),np.eye(4))
        # nib.save(new_img,'/data1/lsm/voxelmorph-oasis/try_test_image/seg_warped.nii.gz')

        # compute volume overlap (dice)
        # overlap = vxm.py.utils.dice(warped_seg, fixed_seg, labels=labels)
        # dice_sep.append(np.array(overlap))
...
# np.savetxt('/data1/lsm/voxelmorph-oasis/avg_dice.csv', np.array(dice_means), delimiter=',')
# np.savetxt('/data1/lsm/voxelmorph-oasis/sep_dice.csv', np.array(dice_sep), delimiter=',')
