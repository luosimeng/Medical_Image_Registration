...
parser.add_argument('--steps-per-epoch', type=int, default=100,
                    help='frequency of model saves (default: 100)')  # 100  580
...
if args.atlas:
    ...
    generator = vxm.generators.scan_to_atlas(atlas,
    ...
    generator = vxm.generators.scan_to_scan(
        batch_size=args.batch_size, bidir=args.bidir, add_feat_axis=add_feat_axis)

# extract shape and number of features from sampled input
# sample_shape = next(generator)[0][0].shape
sample_shape = [1, 160, 192, 224, 1]
...
csvlogger = tf.keras.callbacks.CSVLogger(filename='/data1/lsm/voxelmorph-oasis/epoch_log.csv', separator=',', append=False)
...
def plot_history(hist, loss_name='loss'):
    ...
    plt.xlabel('epoch')
    # plt.show()
    plt.savefig('/data1/lsm/voxelmorph-oasis/loss.png')
