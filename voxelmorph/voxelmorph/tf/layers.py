...
class SpatialTransformer(Layer):
    ...
    # map transform across batch
        if self.single_transform:
            ...
            return tf.map_fn(self._single_transform, [vol, trf], dtype=vol.dtype)#fn_output_signature=vol.dtype)
...
class VecInt(Layer):
    ...
    def call(self, inputs):
        ...
        out = tf.map_fn(self._single_int,
                        [loc_shift] + inputs[1:],
                        dtype=tf.float32)
                        #fn_output_signature=loc_shift.dtype)      #去掉了参数：fn_output_signature=loc_shift.dtype
        if hasattr(inputs[0], '_keras_shape'):
