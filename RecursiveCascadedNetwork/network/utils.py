...
class MultiGPUs:     #self.predictions, self.adamOpt = gpus(self.network, net_pls, opt=adamOptimizer)
    ...
    def __call__(self, net, args, opt=None):
        ...
        for i in range(self.num):  #num:devices
            def auto_gpu(opr):
            ...
            with tf.device(auto_gpu):
                ...
                net.controller = self
                result = net(*[arg[i] for arg in args])   #核心！！！
                results.append(result)
                if opt is not None:   #opt=adamOptimizer
