...
#     seg1 = tf.cast(seg1 > 128, tf.float32)  #灰度值>128的为1，小于的为0
...
#     '''
#     sizes = np.prod(seg1.shape.as_list()[1:])
#     # print('seg1.shape.1',seg1.shape)
#     # seg1 = tf.reshape(seg1, [-1, sizes])
#     # seg2 = tf.reshape(seg2, [-1, sizes])
#     # print('seg1.shape.2:',seg1.shape)
#     seg2 = tf.round(seg2) #四舍五入，0~35
#     # print('seg2 AFTER ROUND:',tf.unique(seg2))
#     dice_all=[]
#     jacc_all=[]
#     for huidu in range(1,36):
#         # dice_score=binary.dc(seg1==huidu,seg2==huidu)
#         # jacc_score=binary.jc(seg1==huidu,seg2==huidu)
#         seg1_h=(seg1==huidu)
#         seg2_h=(seg2==huidu)
#         dice_score = 2.0 * tf.reduce_sum(seg1_h * seg2_h, axis=-1) / (tf.reduce_sum(seg1_h, axis=-1) + tf.reduce_sum(seg2_h, axis=-1))
#         jacc_score=1
#         dice_all.append(dice_score)
#         jacc_all.append(jacc_score)
#     return (tf.reduce_mean(dice_all), tf.reduce_mean(jacc_all))



class RecursiveCascadedNetworks(Network):  #最核心！返回值即包括framework.py里的prediction
    default_params = {
        'weight': 1,
        'raw_weight': 1,
        'reg_weight': 1,
    }      #predictions["loss"]这样用；要找到想输出的图片变量，添到framework.py的build_summary函数里 
    ...
    def __init__(self, name, framework,
        ...
        # self.seg_reconstruction = vxm.networks.Transform([128,156,180], interp_method='nearest')
    ...
    def build(self, img1, img2, seg1, seg2, pt1, pt2):  
        ...
        if self.framework.segmentation_class_value is None:    #None
            seg_fixed = seg1
            warped_seg_moving = self.reconstruction([seg2, flow])  
            # dice_score, jacc_score = mask_metrics(seg_fixed, warped_seg_moving)
            # warped_seg_binary = tf.cast(warped_seg_moving > 128, tf.float32)
            # warped_seg_binary = tf.round(warped_seg_moving)
            # jaccs = [jacc_score]
            # dices = [dice_score]
        else:
        ...
        ret.update({'loss': tf.reshape(loss, (1, )),
                    ...
                    # 'warped_seg_binary':warped_seg_binary,
                    'image_fixed': img1,
                    'image_moving':img2,
