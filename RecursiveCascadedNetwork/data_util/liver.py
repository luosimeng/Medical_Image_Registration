...
class Dataset:
    def __init__(self, split_path, affine=False, mask=False, paired=False, task=None, batch_size=None):
        ...
        for k, v in config['subsets'].items():  #k="combine-train",v=[文件名]
            self.subset[k] = {}
            for entry in v:
    ...
    def get_pairs(self, data, ordered=True):
        ...
        for i, d1 in enumerate(data):
            # j,d2=choice(list(enumerate(data)))
            # if i!=j:
            #     pairs.append((d1,d2))
    ...
    def generator(self, subset, batch_size=None, loop=False):
        ...
        scheme = self.schemes[subset]  #scheme--"sliver_val":1.0
        if 'registration' in self.task:
            ...
            while True:
                # ret = dict()
                # ret['voxel1'] = np.zeros(
                #     (batch_size,160,192,224, 1), dtype=np.float32)
                # ret['voxel2'] = np.zeros(
                #     (batch_size,160,192,224, 1), dtype=np.float32)
                # ret['seg1'] = np.zeros(
                #     (batch_size, 160,192,224, 1), dtype=np.float32)
                ...
                f=h5py.File('/media/gdp/work/lsm/full_cascaded_oasis/datasets/oasis_test.h5')
                with open("/media/gdp/work/lsm/full_cascaded_oasis/Pair_list.txt", "r") as ftxt:
                    for line in ftxt.readlines():
                        ...
                        # print('***Test Pair: moving%s-fixed%s***' % (index_m,index_f))
                        ret['voxel1'][i, ..., 0], ret['voxel2'][i, ...,0] = f[index_f]['volume'],f[index_m]['volume']
                        ret['seg1'][i, ..., 0] = f[index_f]['segmentation']
                        ret['seg2'][i, ..., 0] = f[index_m]['segmentation']
                        ret['id1'][i] = index_f
                        ret['id2'][i] = index_m
                        yield ret
