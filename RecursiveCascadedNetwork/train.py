...
parser.add_argument('--val_steps', type=int, default=100)#100
...
def main():
    ...
    Framework.net_args['base_network'] = args.base_network  #default=VTN
    Framework.net_args['n_cascades'] = args.n_cascades
    Framework.net_args['rep'] = args.rep    #default=1
    Framework.net_args.update(eval('dict({})'.format(args.net_args)))
    with open(os.path.join(args.dataset), 'r') as f:
        cfg = json.load(f)
        image_size = cfg.get('image_size', [160,192,224])
    ...
    with tf.Session(config=Config) as sess:
        ...
        while True:
            ...
            if args.debug or steps % 10 == 0:
                if steps >= args.epochs * iterationSize:  #共训练epochs*iterationSize个step
                    break

                if not args.debug:
                    summaryWriter.add_summary(summ, steps)

                if steps % 10 == 0:   #原来是1000
                ...
                if args.debug or steps % args.val_steps == 0:
                    ...
                    #         steps, sess, val_gen, summary=True)
                    ...
                    #     print('step-',steps,' : validation finished!')
                    # except:
                    #     if steps == args.val_steps:
                    #         print('Step {}, validation failed!'.format(steps))
                    val_gen = dataset.generator(Split.VALID, loop=False, batch_size=batchSize)
                    metrics = framework.validate(steps, sess, val_gen, summary=True)
                    
                    # try_image=framework['warped_moving']
                    # image_save=nib.Nifti1Image(try_image,np.eye(4))
                    # nib.save(image_save, '/data1/lsm/my_casacded/output_images/'+str(steps)+'moved.nii.gz')
