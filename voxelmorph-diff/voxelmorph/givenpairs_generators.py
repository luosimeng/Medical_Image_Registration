...
def volgen_moving(
    ...
    step_in_epoch=0
    path_list=[]
    with open('/media/gdp/work/lsm/voxelmorph-given-pairs/movivng_list.txt', "r") as f:
    ...
    while True:
        ...
        imgs = [py.utils.load_volfile(path_list[step_in_epoch], **load_params)]
        vols = [np.concatenate(imgs, axis=0)]
        # print('\nstep-',step_in_epoch,'-moving: ',path_list[step_in_epoch][65:69])
        
        if (step_in_epoch==579):
            step_in_epoch=0
        else:
            step_in_epoch += 1
...
def volgen_fixed(
    ...
    step_in_epoch=0
    path_list=[]
    with open('/media/gdp/work/lsm/voxelmorph-given-pairs/fixed_list.txt', "r") as f:
        for line in f.readlines():
            line = line.strip('\n')
            path_list.append(line)
    # print(path_list)
    ...
    while True:
        ...
        imgs = [py.utils.load_volfile(path_list[step_in_epoch], **load_params)]
        vols = [np.concatenate(imgs, axis=0)]
        # print('step-',step_in_epoch,'-fixed: ',path_list[step_in_epoch][65:69])
...
def scan_to_scan(bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    ...
    """
    zeros = None
    gen_moving = volgen_moving(batch_size=batch_size, **kwargs)
    gen_fixed = volgen_fixed(batch_size=batch_size, **kwargs)
    while True:
        ...
        invols = [scan1, scan2]     #scan1 -> moving
