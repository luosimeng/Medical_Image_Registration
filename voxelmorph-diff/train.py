...
parser.add_argument('--img-list', default='Train_list.txt', help='line-seperated list of training files')
...
# build the model
model = vxm.tf.networks.VxmDense(
    inshape=inshape,
...
if args.load_weights:
    model.load_weights(args.load_weights)  #args.load_weights

# prepare image loss
if args.image_loss == 'ncc':
    image_loss_func = vxm.tf.losses.NCC().loss
elif args.image_loss == 'mse':
    image_loss_func = vxm.tf.losses.MSE(args.image_sigma).loss
else:
...
if args.use_probs:
    ...
    losses += [vxm.tf.losses.KL(args.kl_lambda, flow_shape).loss]
else:
    losses += [vxm.tf.losses.Grad('l2', loss_mult=args.int_downsize).loss]

weights += [args.lambda_weight]

# multi-gpu support
if nb_devices > 1:
    save_callback = vxm.tf.networks.ModelCheckpointParallel(save_filename)
...
#speed up!!!
#ori :model.compile(optimizer=tf.keras.optimizers.Adam(lr=args.lr), loss=losses, loss_weights=weights)
...
csvlogger = tf.keras.callbacks.CSVLogger(filename='/media/gdp/work/lsm/voxelmorph-liver/epoch_log.csv', separator=',', append=False)

history = model.fit_generator(generator,
                    initial_epoch=args.initial_epoch,
                    epochs=args.epochs,
                    steps_per_epoch=args.steps_per_epoch,
                    callbacks=[save_callback,csvlogger],
                    verbose=1
                    )
model.summary()
...
def plot_history(hist, loss_name='loss'):
    ...
    plt.xlabel('epoch')
    # plt.show()
    plt.savefig('/media/gdp/work/lsm/voxelmorph-liver/loss.png')
